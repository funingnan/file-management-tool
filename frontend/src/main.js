...
const VERSION_HISTORY = [
    {
        ...
        date: '2026-08-11',
        changes: [
            '新增扫描层级控制：1 层（仅当前目录）/ 2 层 / 3 层 / 全部层级',
...
async function loadFileTags(docId) {
    try {
        ...
        if (detail.tags && detail.tags.length > 0) {
            const html = detail.tags.map(t => `<span class="file-tag" style="background:${getTagColor(t)}20;color:${getTagColor(t)};border-color:${getTagColor(t)}40">${escapeHtml(t.name)}</span>`).join('');
